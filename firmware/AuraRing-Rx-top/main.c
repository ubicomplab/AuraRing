...
#define BUFFER_SIZE 4       //Buffer size for LPF, only powers of two
...
unsigned char bottomBoard_buffer[9];
uint8_t bottomBoard_index = 0;

long aclk =0;
long mclk =0;
long smclk =0;

void low_pass(int16_t channel_a[][3], int16_t channel_b[][3]) {
    int32_t sum_a[3]={0,0,0};
    int32_t sum_b[3]={0,0,0};
    uint8_t i=0; uint8_t j=0;
    for (j=3; j > 0; j--) {
        for (i=BUFFER_SIZE; i > 0; i--) {
            sum_a[j-1] += channel_a[i-1][j-1];
            sum_b[j-1] +=channel_b[i-1][j-1];
        }
        filter[(j-1)*2] = sum_a[j-1] >> 2;
        filter[(j-1)*2+1] = sum_b[j-1] >> 2;
...
//    timer_init();
	...
	while (1) {
        ...
        ////        Timer_A_startCounter(TIMER_A0_BASE, TIMER_A_UP_MODE);
                ...
                adc_a[buffer_count-1][adc_channel-1] = ((sampled >> 18) & 0xFFF) << 4; // 12bit ADC
                adc_a[buffer_count-1][adc_channel-1] = adc_a[buffer_count-1][adc_channel-1] >> 4;

                P1OUT |= BIT7;
            }
	    }
	    low_pass(adc_a, adc_b);
#ifdef USE_BOTTOM
	    // recieve from bottom board
        EUSCI_A_UART_enableInterrupt(EUSCI_A0_BASE,EUSCI_A_UART_RECEIVE_INTERRUPT);
	    while (bottomBoard_index <= 8) {
            __bis_SR_register(LPM0_bits | GIE);   // enable global interrupts, enter LPM0
            if (bottomBoard_index == 0) {
                if (bottomBoard_buffer[0] != 0x55 ) {
                    continue;
                }
            }
            if (bottomBoard_index == 1) {
                if (bottomBoard_buffer[1] != 0x57) {
                    bottomBoard_index = 0;
                    continue;
                }
            }
        ...
        for (adc_channel = 3; adc_channel > 0; adc_channel--)
        {
            EUSCI_A_UART_transmitData(EUSCI_A0_BASE, (filter[(adc_channel-1)*2] & 0xFF0)>>4);
            EUSCI_A_UART_transmitData(EUSCI_A0_BASE, ((filter[(adc_channel-1)*2] & 0x00F)<<4)+((filter[(adc_channel-1)*2 + 1] & 0xF00)>>8));
            EUSCI_A_UART_transmitData(EUSCI_A0_BASE, (filter[(adc_channel-1)*2 + 1] & 0xFF)>>0);
        }
        uint8_t counter_bottomBoard;
        for (counter_bottomBoard = 2; counter_bottomBoard < 9; counter_bottomBoard++)
        {
            EUSCI_A_UART_transmitData(EUSCI_A0_BASE, bottomBoard_buffer[counter_bottomBoard]);
        }
        __no_operation();                     // For debug,Remain in LPM0
        frame_num ++;
...
#endif
{
  switch(__even_in_range(UCA0IV,USCI_UART_UCTXCPTIFG))
  {
    ...
    case USCI_UART_UCRXIFG:
      bottomBoard_buffer[bottomBoard_index] = UCA0RXBUF;
      __no_operation();
